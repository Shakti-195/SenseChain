...
const PrivateRoute = ({ children }) => {
  ...
  if (loading) return (
    <div className="h-screen w-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-[#090909] font-bold text-slate-400 dark:text-red-500">
      <div className="w-12 h-12 border-[3px] border-t-red-600 border-slate-200 dark:border-red-900 rounded-full animate-spin mb-5" />
...
function AppContent() {
  ...
  useEffect(() => {
    ...
    const connectWS = () => {
      ...
      const socketUrl = import.meta.env.VITE_WS_URL || (isProd ? "wss://sensechain.onrender.com/ws" : "ws://127.0.0.1:8000/ws");
      ...
      socket.onmessage = (event) => {
        try {
          ...
          if (data.type === "UPDATE") {
            setChainData(prev => {
              if (prev.integrity === true && data.integrity === false) playSecurityBreachAlarm();
              return {
                chain: data.chain || [],
          ...
          <Suspense fallback={
            <div className="flex flex-col items-center justify-center h-full p-20 animate-pulse">
              <div className="w-12 h-12 border-2 border-t-transparent rounded-full animate-spin mb-4"
                   style={{ borderColor: 'var(--th-glow)', borderTopColor: 'var(--th-primary)' }} />
